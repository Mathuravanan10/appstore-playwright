...
test.describe('multiple check testcase function', async () => {
  ...
  test('SapUi5 test',async () => {
    ...
    for(const click of managepurchaseorder){
      ...
      await page.waitForTimeout(8000);
      const currency = await page.locator('ui5-xpath=//sap.m.Input[@id="ui.ssuite.s2p.mm.pur.po.manage.st.s1::sap.suite.ui.generic.template.ObjectPage.view.Details::C_PurchaseOrderTP--GeneralInformationFacet1::DocumentCurrency::Field-input"]');
